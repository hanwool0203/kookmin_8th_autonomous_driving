...
class PurePursuitVisualizerNode(Node):
    ...
    """
    def __init__(self):
        super().__init__('pure_pursuit_visualizer_node_v004')
...
# --- 8, 9, 10 단계: 주행 경로 및 조향각 결정 (통합된 조건문) ---
        ...
        angle_cmd = 0.0  # 기본 조향각은 0으로 초기화
        interp_x = None  # 경로 초기화

        # 조건 1: 정상 주행 (양쪽 콘이 2개 이상)
        ...
        # 조건 2: 왼쪽 콘 없음 (가상 경로 생성)
        elif len(left_cones) == 0 and 0 < len(right_cones) < 4:
            virtual_left_cones = self.create_virtual_cones(right_cones, 'left', 1.0)
            if virtual_left_cones:
                left_cones = virtual_left_cones
                midpoints = self.calculate_midpoints(left_cones, right_cones)
                interp_x, interp_y = self.interpolate_path(midpoints)
        
        # *** 신규 추가된 조건 3: 단일 쌍만 감지 ***
    ...
    def create_virtual_cones(self, real_cones, direction, track_width):
        """
        감지된 실제 콘을 기반으로 반대편에 가상의 콘을 생성합니다.
        :param real_cones: 실제 감지된 콘 리스트
        :param direction: 생성할 가상 콘의 방향 ('left' 또는 'right')
        :param track_width: 사용할 트랙 폭
        :return: 가상 콘 리스트
        """
        if len(real_cones) < 2:
            return []

        # 1. 실제 콘들로 라인 피팅 (1차 다항식, 즉 직선)
        cone_x = np.array([p[0] for p in real_cones])
        cone_y = np.array([p[1] for p in real_cones])
        # y를 x에 대한 함수로 피팅: y = m*x + b
        m, b = np.polyfit(cone_y, cone_x, 1)

        virtual_cones = []
        for x_real, y_real in real_cones:
            # 2. 법선 벡터 계산
            # 라인의 기울기(m)로부터 법선 벡터 (-m, 1) 또는 (m, -1)을 구함
            # 이 벡터를 정규화하여 단위 벡터로 만듦
            normal_vector = np.array([-m, 1])
            normal_vector /= np.linalg.norm(normal_vector)

            # 3. 평행 이동
            offset = track_width if direction == 'left' else -track_width
            
            # y값의 변화에 따라 법선 방향이 뒤집히는 것을 방지
            # 여기서는 왼쪽 콘을 생성하므로 x값을 더 작은 쪽으로 이동
            if normal_vector[0] > 0:
                 normal_vector *= -1

            vx = x_real + normal_vector[0] * offset
            vy = y_real + normal_vector[1] * offset
            virtual_cones.append((vx, vy))
